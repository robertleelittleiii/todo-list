...
cur = new Date().getTime();
 ...
 function getInputSize(minWidth,comfortZone,inputField) 
 {
        ...
        var minWidth = minWidth || $(inputField).width();
            val = '';
            input = $(inputField);
    ...
    $(inputField).filter('input:text').each(function(){

        var minWidth = o.minWidth || $(inputField).width(),
            val = '',
            input = $(inputField),
            testSubject = $('<tester/>').css({
                position: 'absolute',
...
BestInPlaceEditor.forms = {
 ...
 //console.log(getInputSize("40","10",this.element.find('input')));
              newWidth=((this.sanitizeValue(this.oldValue).length)*6)+5;
              if (newWidth < 40) newWidth = 40;
              
              this.element.find('input').width(newWidth);
 ...
 //console.log(this.element.find('input'))
        ...
        keydownHandler : function(event) {
            ...
            if (event.keyCode == 9) {
                ...
                //  $(this).parent().next().next().find(".best_in_place").trigger('click');
                //    $(this).parent().parent().parent().next().next().find(".best_in_place").trigger('click');
               list=$(".best_in_place");
               thisID=jQuery.inArray($(this).parent().parent()[0],list);
               
              
                if (event.shiftKey )
                {
                   // eval("$(this).parent().parent().parent().prev().prev().prev().find('.best_in_place').trigger('click')")
                  //  console.log($(this).parent().parent().parent().prev().prev().prev().find(".best_in_place").trigger('click'));
                    //console.log($(this).parent().parent().parent().prev().prev().find(".best_in_place").trigger('click'));
                // the wait function is a fix for a bug with safari
                    $(this).focus();
                     wait(10);
                    $(this).caretToEnd();
                    wait(10);
                    $(list[thisID-1]).trigger('click');
                }
                else
                {
                //    console.log($(this).parent().parent());
                 //   console.log($(list[0]));
                  // console.log( thisID=jQuery.inArray($(this).parent().parent()[0],list)+1)
                  //  console.log($(list[thisID]).trigger('click'));
                  console.log($(this));
                     $(this).focus();
                     wait(10);
                    $(this).caretToEnd();
                    ...
                    $(list[thisID+1]).trigger('click');
                    wait(10);
     //               console.log($(this).parent().parent().parent().next().next().next().find(".best_in_place").trigger('click'));
     //               console.log( $(this).parent().parent().parent().next().next().find(".best_in_place").trigger('click'));

                }
                   
            //   $(this).next('.best_in_place').activate();
